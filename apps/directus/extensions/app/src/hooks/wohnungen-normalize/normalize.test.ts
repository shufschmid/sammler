...
describe('normalizeWohnungPayload', () => {
  ...
  it('marks over-350 and wrong-type as not passing', () => {
    expect(
      normalizeWohnungPayload(
        { miete_chf: 2100, flaeche_m2: 70, plz: '4051' },
        true
      ).ai_passt
    ).toBe(false)
    expect(
      normalizeWohnungPayload(
        { ai_wg: true, miete_chf: 800, flaeche_m2: 40, plz: '4051' },
        true
      ).ai_passt
    ).toBe(false)
  })
  ...
  it('recomputes on an update that carries both rent and area', () => {
    const n = normalizeWohnungPayload(
      { miete_chf: 1500, flaeche_m2: 90, plz: '4052' },
      false
    )
